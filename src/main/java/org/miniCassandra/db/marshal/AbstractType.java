...
public abstract class AbstractType<T> implements Comparator<ByteBuffer>
{
    ...
    protected AbstractType(ComparisonType comparisonType)
    {
        ...
        try
        {
            ...
            if ((custom.getDeclaringClass() == AbstractType.class) == (comparisonType == ComparisonType.CUSTOM))
                throw new IllegalStateException((comparisonType == ComparisonType.CUSTOM ? "compareCustom must be overridden if ComparisonType is CUSTOM"
    ...
    /**
     * Validate cell value. Unlike {@linkplain #validate(ByteBuffer)},
